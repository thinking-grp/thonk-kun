...
export function getTokenGroupById(id: string): database.TokenGroup[] {
  ...
  const result: database.TokenGroup[] = [];
...
export function editTokenGroupById(
  id: string,
  editedTokenGroup: database.TokenGroup
) {
  const dict: database.TokenGroupDic = database.getTokenGroupDic();
...
export function addWhatTokens(tokenId: string, negaposi: number) {
  const tokenGroups = getTokenGroupById("tng-what");
...
export function isWhatToken(token: database.Token): boolean {
  const dict: database.TokenGroup[] = getTokenGroupById("tng-what");
...
export function includesInTokenGroup(
  tokenGroupId: string,
